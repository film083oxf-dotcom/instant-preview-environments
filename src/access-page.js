export function renderAccessPage({ repository, user, memberships, projectMemberships, quota, resourceUsage }) {
  const projectName = String(repository || "").split("/")[1] || String(repository || "Current project");
  const quotaValues = quota || { max_active_environments: 10, max_active_databases: 10, max_concurrent_builds: 3 };
  const usage = resourceUsage || { activeEnvironments: 0, activeDatabases: 0, concurrentBuilds: 0, activeReservations: 0 };
  ...
  return "<!doctype html><html lang='en'><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><title>Access Control · Instant Preview</title>" +
    ...
    "<section class='panel'><h2>Resource quotas · " + escapeHtml(projectName) + "</h2><p class='subtitle'>Limits are enforced before a new Preview Environment provisions its isolated D1.</p><div class='note'>Current usage: <strong>" + usage.activeEnvironments + " / " + quotaValues.max_active_environments + " active environments</strong> · <strong>" + usage.activeDatabases + " / " + quotaValues.max_active_databases + " databases</strong> · <strong>" + usage.concurrentBuilds + " / " + quotaValues.max_concurrent_builds + " concurrent builds</strong> · reservations " + usage.activeReservations + ".</div><form class='form-row' method='post' action='/api/resource-quota/update'><div class='field'><label>Max active environments</label><input type='number' min='1' max='1000' name='max_active_environments' value='" + escapeAttr(quotaValues.max_active_environments) + "' required></div><div class='field'><label>Max active databases</label><input type='number' min='1' max='1000' name='max_active_databases' value='" + escapeAttr(quotaValues.max_active_databases) + "' required></div><div class='field'><label>Max concurrent builds</label><input type='number' min='1' max='1000' name='max_concurrent_builds' value='" + escapeAttr(quotaValues.max_concurrent_builds) + "' required></div><button class='primary' type='submit'>Save quotas</button></form></section>" +
    "<p class='note'>Phase 10 adds project-level resource quotas so Preview environments have an explicit capacity boundary. Existing environments are not interrupted when the quota is reached.</p>" +
    "</main></body></html>";
}
