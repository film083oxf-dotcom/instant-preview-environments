...
export async function getDashboardData(env) {
  ...
  const query = "SELECT pr_number, status, commit_sha, preview_url, database_name, ai_status, updated_at, last_error FROM environments ORDER BY datetime(updated_at) DESC LIMIT 50";
  const { results } = await env.CONTROL_DB.prepare(query).all();
  ...
  const environments = (results || []).map(row => ({
    ...
    prUrl: "https://github.com/" + env.GITHUB_REPO + "/pull/" + row.pr_number,
...
export function renderDashboard(data, user = null) {
  ...
  return '<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Instant Preview Dashboard</title><style>' +
    ...
    '<div class="top"><div><div class="eyebrow">Control Plane</div><h1>Instant Preview Dashboard</h1><p class="subtitle">PR → Preview → isolated database → AI diagnosis → cleanup</p></div><div class="account">' + (user ? 'Signed in as <strong>' + escapeHtml(user.login) + '</strong>' + (user.role === 'admin' ? ' · <a href="/access">Access Control</a>' : '') + ' · <a href="/auth/logout">Sign out</a>' : '<a href="/auth/login">Sign in with GitHub</a>') + '</div><div class="refresh">Updated ' + escapeHtml(new Date(data.generatedAt).toLocaleString()) + '</div></div>' +
    ...
    '<footer>Repository: ' + escapeHtml(data.repository) + ' · State stored in central D1 control plane.</footer>' +
    '</main></body></html>';
}
