import { getProjectResourceUsage } from "./resource-control.js";
...
export async function getDashboardData(env, projectId = null) {
  ...
  const [environmentRows, resource] = await Promise.all([
    env.CONTROL_DB.prepare(query).bind(project.project_id).all(),
    getProjectResourceUsage(env.CONTROL_DB, project.project_id)
  ]);
  const { results } = environmentRows;
...
export function renderDashboard(data, user = null, projects = []) {
  ...
  return '<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Instant Preview Dashboard</title><style>' +
    ...
    '<section class="summary">' + metric("Active", data.summary.active) + metric("Ready", data.summary.ready) + metric("Building", data.summary.building) + metric("Failed", data.summary.failed) + metric("Deleted", data.summary.deleted) + metric("Capacity", data.resource.quota ? data.resource.activeEnvironments + " / " + data.resource.quota.max_active_environments : "—") + '</section>' +
    ...
    '<section class="resource-panel"><div><div class="eyebrow">Resource Management</div><h3>Project capacity</h3><p>Active environments <strong>' + data.resource.activeEnvironments + ' / ' + (data.resource.quota?.max_active_environments ?? "—") + '</strong> · Active databases <strong>' + data.resource.activeDatabases + ' / ' + (data.resource.quota?.max_active_databases ?? "—") + '</strong> · Concurrent builds <strong>' + data.resource.concurrentBuilds + ' / ' + (data.resource.quota?.max_concurrent_builds ?? "—") + '</strong> · Reservations <strong>' + data.resource.activeReservations + '</strong></p></div><a class="button" href="/access">Manage quotas</a></section>' +
