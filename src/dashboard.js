...
export function renderDashboard(data, user = null) {
  ...
  return '<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Instant Preview Dashboard</title><style>' +
    ...
    '<div class="top"><div><div class="eyebrow">Control Plane</div><h1>Instant Preview Dashboard</h1><p class="subtitle">PR → Preview → isolated database → AI diagnosis → cleanup</p></div><div class="account">' + (user ? 'Signed in as <strong>' + escapeHtml(user.login) + '</strong> · <a href="/auth/logout">Sign out</a>' : '<a href="/auth/login">Sign in with GitHub</a>') + '</div><div class="refresh">Updated ' + escapeHtml(new Date(data.generatedAt).toLocaleString()) + '</div></div>' +
